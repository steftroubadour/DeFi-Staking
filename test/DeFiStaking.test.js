...
const DeFiStaking = artifacts.require("./DeFiStaking.sol");
...
contract("DeFiStaking", async accounts => {
  const owner = accounts[0];
  const user = accounts[1];

  let DeFiStakingInstance
  ...
  let tokens = []
  const addToken = (_name, _symbol, _instance, _tokenPriceFeedInstance, _yield) => {
    tokens[_symbol] = {
      name: _name,
      symbol: _symbol,
      instance: _instance,
  ...
  const newERC20TokenAndPriceFeed = async (_name, _symbol, _price, _yield) => { // _price: précision à 8 chiffres
    const _tokenInstance = await ERC20Token.new(_name, _symbol, { from: owner })
    const _tokenPriceFeedInstance = await ChainLinkPriceFeed.new(_price* 10 ** 8, 8)
  ...
  const initialSetUp = async () => {
    ...
    DeFiStakingInstance = await DeFiStaking.new(  // Deploy in truffle test env
    ...
    await transfert(rewardTokenSymbol, DeFiStakingInstance.address, etherToWei(10))
  }
  ...
  describe("Initial State", () => {
    ...
    it("contract balance is correct", async () => {
      let balance = await tokens[rewardTokenSymbol].instance.balanceOf(DeFiStakingInstance.address)
      assert.strictEqual(balance.toString(), etherToWei(10))
    })
    it("No event 'PoolAdded' emitted", async () => {
      const result = await truffleAssert.createTransactionResult(DeFiStakingInstance, DeFiStakingInstance.transactionHash);
      truffleAssert.eventNotEmitted(result, 'PoolAdded');
    })
    it("one pool registered", async () => {
      const poolsAddresses = await DeFiStakingInstance.getPools()
      assert.isArray(poolsAddresses)
      assert.include(poolsAddresses, tokens[rewardTokenSymbol].instance.address)
      assert.lengthOf(poolsAddresses, 1)
    })
    it("reward token pool is the good one", async () => {
      const token = await DeFiStakingInstance.rewardToken()
      assert.strictEqual(token, tokens[rewardTokenSymbol].instance.address)
    })
    it("registered pool is reward token pool", async () => {
      const pool = await DeFiStakingInstance.pools(tokens[rewardTokenSymbol].instance.address)

      assert.strictEqual(pool.token, tokens[rewardTokenSymbol].instance.address)
      assert.strictEqual(pool.yield, tokens[rewardTokenSymbol].yield.toString())
      assert.strictEqual(pool.priceFeed, tokens[rewardTokenSymbol].priceFeedInstance.address)
    })
    it("last reward token price is 1", async () => {
      const lastPrice = await DeFiStakingInstance.getLastPrice(tokens[rewardTokenSymbol].instance.address)
    ...
    tests.forEach((value) => {
      ...
      it(`user[${accounts[_index]}] balance in pool is zero`, async () => {
        const balance = await DeFiStakingInstance.getUserBalanceInPool(tokens[rewardTokenSymbol].instance.address, accounts[_index])
  ...
  describe("#addPool()", () => {
    it("Should be called only by owner", async () => {
      await truffleAssert.reverts(DeFiStakingInstance.addPool(
    ...
    it("new pool is added", async () => {
      await DeFiStakingInstance.addPool(
        tokens["LINK"].instance.address,
        tokens["LINK"].priceFeedInstance.address,
        tokens["LINK"].yield,
        { from: owner }
  ...
  const approveAndStake = async (_tokenSymbol, _amount, _from) => {
    await tokens[_tokenSymbol].instance.approve(
      DeFiStakingInstance.address,
      etherToWei(_amount),
      { from: _from }
    )
    return DeFiStakingInstance.stake(
  ...
  describe("#stake()", () => {
    it("Non valid amount", async () => {
      await truffleAssert.reverts(DeFiStakingInstance.stake(
        tokens[rewardTokenSymbol].instance.address,
        0,
        { from: owner }),
        "Non valid amount"
    ...
    it("new user pool balance expected", async () => {
      const balance = await DeFiStakingInstance.getUserBalanceInPool(tokens[rewardTokenSymbol].instance.address, owner)
      assert.strictEqual(balance.toString(), etherToWei(10))
    })
    before('SETUP', async () => {
      await initialSetUp()
  ...
  const waitingBeforeUnstake = async (_time) => {
    const lastStakeBlock = await web3.eth.getBlock(lastStakeResult.receipt.blockNumber) // Retrieve timestamp of the last stake
  ...
  describe("#pendingReward()", () => {
    ...
    it("Must have pending rewards", async () => {
      const pendingRewards = await DeFiStakingInstance.pendingReward(tokens[rewardTokenSymbol].instance.address, { from: owner })
      assert.isTrue(pendingRewards.toNumber() > 0)
      assert.strictEqual(
        pendingRewards.toNumber(),
        Math.floor(stakedAmount * stakedTime * tokens[rewardTokenSymbol].yield / 100 / 365 / 24 / 60 / 60)
      )
    })
  })
  ...
  describe("#Unstake()", () => {
    before('SETUP', async () => {
      ...
      lastStakeResult = await approveAndStake(rewardTokenSymbol, 10, owner)
      await waitingBeforeUnstake(stakedTime)
      await DeFiStakingInstance.unstake(tokens[rewardTokenSymbol].instance.address, { from: owner })
    })
    it("new user pool balance expected", async () => {
      const balance = await DeFiStakingInstance.getUserBalanceInPool(tokens[rewardTokenSymbol].instance.address, owner)
      assert.strictEqual(balance.toString(), '0')
